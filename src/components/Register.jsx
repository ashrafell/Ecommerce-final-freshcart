...
export default function Register() {
    ...
    /*  function validation(values) {
          ...
          if (!values.name)
              ...
              errors.name = 'name should be in the format of first letter capital and 3-5'
          if (!values.email)
    ...
    let validationSchema = yup.object({
        name: yup.string().min(2, 'name must be at min 2 words').max(10, 'name must be at max 2 words').required('name is Required'),
        email: yup.string().email('Invalid email address').required('email is Required'),
        password: yup.string().matches(/^[A-Z][a-z0-9]{2,5}$/).required('password is Required'),
        rePassword: yup.string().oneOf([yup.ref('password')], 'rePassword must be same password').required('rePassword is Required'),
        phone: yup.string().matches(/^01[0-25][0-9]{8}$/).required('phone must be same password')
    })
    ...
    return (
        ...
        <div>
            <h2 className='ms-96 fa-2x'>Register Now :</h2>
            {msg ? <div className="p-4 mb-4 text-sm W-1/2 mx-auto text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                <span className="font-medium">{msg}</span>
            </div> : ''}

            <form className="max-w-md mx-auto mt-4 " onSubmit={formik.handleSubmit}>
                ...
                <div className="relative z-0 w-full mb-5 group">
                    ...
                    <label htmlFor="floating_email" className="peer-focus:font-medium absolute mt-2 text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-green-600 peer-focus:dark:text-green-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Email address</label>
                </div>
                ...
                <div className="relative z-0 w-full mb-5 group">
                    <input onBlur={formik.handleBlur} value={formik.values.rePassword} onChange={formik.handleChange} type="rePassword" id="rePassword" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-green-500 focus:outline-none focus:ring-0 focus:border-green-600 peer" placeholder=" " />
